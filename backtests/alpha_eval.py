...
Usage:
    ...
    python backtests/alpha_eval.py --run-id 20260820_152600
"""
import sys
import os
...
def load_prices_and_signals() -> tuple:
    """
    ...
    try:
        ...
        # ML signals (up_proba from price_targets, which MLAlpha writes)
    ...
    signals = pd.DataFrame(signal_rows,
                           columns=['date', 'ticker', 'model_name', 'expected_return', 'confidence'])
    ...
    logger.info(f"Loaded {len(prices)} price dates, "
                f"{all_signals['model_name'].nunique()} models, "
...
def compute_forward_returns(prices: pd.DataFrame, horizon: int) -> pd.DataFrame:
    ...
    NOTE: this uses future prices (shift by -horizon). This is intentional and
    correct here — we are computing actual realized outcomes in the evaluation
    set, not leaking future data into signal computation.
    """
...
def run_alpha_evaluation(
    ...
    output_dir: if provided, save alpha_ic_results.csv there instead of
                (or in addition to) the backtests/ root.
    """
    ...
    if not models:
        logger.warning(f"No models found (filter={model_filter}). "
                       f"Available: {signals['model_name'].unique().tolist()}")
        return
    ...
    # Save to run folder if provided
    if output_dir:
        out_path = os.path.join(output_dir, 'alpha_ic_results.csv')
        out_df.to_csv(out_path, index=False)
        logger.info(f"IC results saved to run folder: {out_path}")

    # Always save / update the root-level flat file for backward compat
    root_out = os.path.join(os.path.dirname(__file__), 'alpha_ic_results.csv')
    out_df.to_csv(root_out, index=False)
    logger.info(f"IC results saved to {root_out}")

    # If no output_dir given, try to find the latest run folder and save there too
    if not output_dir:
        try:
            from backtests.registry import list_runs, RUNS_DIR
            runs = list_runs()
            if runs:
                latest_run_id = runs[0]['run_id']   # list_runs() returns newest first
                latest_dir = RUNS_DIR / latest_run_id
                if latest_dir.exists():
                    ic_path = latest_dir / 'alpha_ic_results.csv'
                    out_df.to_csv(ic_path, index=False)
                    logger.info(f"IC results also saved to latest run: {ic_path}")
        except Exception as e:
            logger.debug(f"Could not auto-save to latest run folder: {e}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Alpha IC evaluation')
    parser.add_argument('--horizon', type=int, default=21)
    parser.add_argument('--model',   default=None,
                        help='Filter to a single model name')
    parser.add_argument('--run-id',  default=None, dest='run_id',
                        help='Save IC results into this run folder')
    args = parser.parse_args()

    out_dir = None
    if args.run_id:
        try:
            from backtests.registry import get_run_dir
            out_dir = str(get_run_dir(args.run_id))
        except Exception as e:
            logger.warning(f"Could not resolve run folder for {args.run_id}: {e}")
