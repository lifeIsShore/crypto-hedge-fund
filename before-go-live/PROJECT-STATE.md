...
15. ~~Sandbox cash double-count bug~~ — ✅ **CODE FIXED 2026-08-10 (session 8).** `paper_trader.py` now writes `cash_history` correctly. **Reconciliation of existing corrupted sandbox data still pending Ahmet's action:** run `FIX_SANDBOX_CASH.bat` (dry-run first) to correct `sandbox_data.db`'s historical `cash_history` rows.
16. **Next up:** The still-open SaaS decision in §6 (API key/data-provider strategy — Ahmet has explicitly deferred this to last). Before that, recommend an end-to-end sandbox dry run (`RUN_SANDBOX.bat`) exercising today's J3/J4/J5 changes, now that both the pipeline crash (session 7/7b) and the cash reconciliation (session 8) are handled.
