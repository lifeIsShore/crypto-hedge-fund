> **STATUS: IMPLEMENTED ✅** — 2026-08-20 (session 11). `backtests/metrics.py` built and import-verified.
> Depends on `01-engine.md` output: `backtests/backtest_results.csv` must exist to run standalone.
> `02-alpha-ic-evaluation.md` runs independently.

# Portfolio Performance Metrics
# `backtests/metrics.py` (new)
...
Given the equity curve from `01-engine.md`, is this strategy worth running?
Compared to just holding MSCI World (`EUNL.DE`)?

The questions in order of importance:
1. Does it beat the benchmark on a **risk-adjusted** basis? (Sharpe, Sortino)
2. How bad did it get? (Max drawdown, Calmar)
3. Is outperformance from genuine alpha or just more risk-taking? (Info Ratio)
4. When did it underperform and by how much? (Underwater equity curve)
5. Is the year-over-year consistency acceptable?
...
Reads backtests/backtest_results.csv produced by 01-engine.
Can also be called as a library: from backtests.metrics import compute_metrics
...
def compute_metrics(df: pd.DataFrame) -> dict:
    ...
    info_ratio = (excess_ret.mean() * TRADING_DAYS) / tracking_error if tracking_error > 0 else 0.0

    # ── Beta / Alpha ──────────────────────────────────────────────────────────
    cov_matrix = np.cov(port_ret.values, bench_ret.values)
    beta = cov_matrix[0, 1] / cov_matrix[1, 1] if cov_matrix[1, 1] > 0 else 1.0
    alpha_annualised = (cagr_port - RISK_FREE_RATE) - beta * (cagr_bench - RISK_FREE_RATE)

    # ── Hit rate (% days portfolio outperforms benchmark) ─────────────────────
    hit_rate = float((excess_ret > 0).mean())

    # ── Turnover ──────────────────────────────────────────────────────────────
    # NOTE: turnover is not directly tracked in backtest_results.csv.
    # It is implicit in the optimizer's TURNOVER_PENALTY × Δweight cost.
    # If you want explicit turnover tracking, add a 'turnover' column to
    # the walk_forward.py output and load it here.
...
## What the metrics mean for your decision

Use these thresholds when reading results. They are calibrated for a long-only
multi-asset EUR portfolio vs. MSCI World, not for a hedge fund with leverage:

| Metric | Stop (system needs work) | Acceptable | Good |
|---|---|---|---|
| Sharpe | < 0.3 | 0.3–0.6 | > 0.7 |
| Info Ratio | < 0.2 | 0.2–0.5 | > 0.5 |
| Max Drawdown | < -40% | -20% to -40% | > -20% |
| Calmar | < 0.2 | 0.2–0.5 | > 0.5 |
| Annual hit rate | < 2/4 years outperform | 3/4 | 4/4 |
| Beta | > 1.2 (just levered market) | 0.8–1.1 | 0.6–0.9 |
| Alpha (ann.) | < -1% | 0–2% | > 2% |

**Important**: this backtest covers **2023–2025**, which is predominantly a bull
market. High Sharpe in this window doesn't prove all-weather robustness.
The 2022 bear market is in the prices table but falls before the momentum warmup
cutoff — it won't be in the backtest results. You can optionally reduce warmup
to 126 days (removes 6M momentum) to extend coverage into late 2022.

---

## Output files

- `backtests/backtest_metrics.csv` — single-row metrics for logging/comparison
- Console table — full formatted output as shown above
